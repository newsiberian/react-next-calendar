...
function getViewNames(_views) {
  return !Array.isArray(_views) ? Object.keys(_views) : _views
}

function isValidView(view, { views: _views }) {
  let names = getViewNames(_views)
...
function Calendar(props) {
  const localizer = React.useMemo(() => {
    const msgs = message(props.messages)
  ...
  }, [props.messages, props.localizer, props.culture, props.formats])

  const getters = React.useMemo(
    () => ({
      eventProp: (...args) =>
        (props.eventPropGetter && props.eventPropGetter(...args)) || {},
      slotProp: (...args) =>
        (props.slotPropGetter && props.slotPropGetter(...args)) || {},
      slotGroupProp: (...args) =>
        (props.slotGroupPropGetter && props.slotGroupPropGetter(...args)) || {},
      dayProp: (...args) =>
        (props.dayPropGetter && props.dayPropGetter(...args)) || {},
    }),
    [
      props.eventPropGetter,
      props.slotPropGetter,
      props.slotGroupPropGetter,
      props.dayPropGetter,
    ]
  )

  const [components, viewNames] = React.useMemo(() => {
    const names = getViewNames(props.views)

    return [
      defaults(
        props.components[props.view] || {},
        omit(props.components, names),
        {
  ...
  }, [props.views, props.view, props.components])

  const accessors = React.useMemo(
    () => ({
      start: wrapAccessor(props.startAccessor),
      end: wrapAccessor(props.endAccessor),
      allDay: wrapAccessor(props.allDayAccessor),
      tooltip: wrapAccessor(props.tooltipAccessor),
      title: wrapAccessor(props.titleAccessor),
      resource: wrapAccessor(props.resourceAccessor),
      resourceId: wrapAccessor(props.resourceIdAccessor),
      resourceTitle: wrapAccessor(props.resourceTitleAccessor),
    }),
    [
      props.startAccessor,
      props.endAccessor,
      props.allDayAccessor,
      props.tooltipAccessor,
      props.titleAccessor,
      props.resourceAccessor,
      props.resourceIdAccessor,
      props.resourceTitleAccessor,
    ]
  )
  ...
  function getViews() {
    ...
    if (typeof props.views === 'object') {
      return mapValues(props.views, (value, key) => {
  ...
  function getDrilldownView(date) {
    if (!props.getDrilldownView) {
      return props.drilldownView
    }

    return props.getDrilldownView(date, props.view, Object.keys(getViews()))
  }
   ...
   * state via url
   */
  function handleRangeChange(date, viewComponent, view) {
    if (props.onRangeChange) {
      if (viewComponent.range) {
        props.onRangeChange(
          viewComponent.range(date, { localizer: props.localizer }),
          view
        )
      } else {
  ...
  function handleNavigate(action, newDate) {
    const { view, date, getNow, onNavigate, ...rest } = props
  ...
  function handleDrillDown(date, view) {
    if (props.onDrillDown) {
      props.onDrillDown(date, view, props.drilldownView)
      return
    }
    if (view) {
